...
function clone(value) {
  const type = is.primitive(value) ? "primitive" : typeOf(value);
...
function merge(a, b, options = {}) {
  ...
  return clone(coerce([b, a].find(is.not.undefined)));
}
...
merge.array = function mergeArrays(a, b, options = {}) {
  ...
  if (options.array === "replace") {
    const defined = [b, a].filter(is.not.undefined);
...
export { clone, coerce, defineValue, getUnique, is, isInstanceOf, merge, typeOf };
